...
const scoreCompetitiveMoat = (d) => {
  ...
  if (hasAny([
    ...
    return { score: 20, label: "Cannot realistically duplicate", explanation: "The available company profile indicates a product, service, technology, platform, or ecosystem that competitors cannot realistically duplicate over the next 2+ years." };
  }
  if (hasAny([
    ...
    return { score: 15, label: "Very difficult to duplicate", explanation: "The available company profile indicates an advantage that would be very difficult for competitors to duplicate over the next 2+ years." };
  }
  if (hasAny([
  ...
  return { score: 0, label: "No verified non-duplicable advantage", explanation: "The available company profile does not verify a product or service that competitors cannot realistically duplicate over the next 2+ years." };
};
...
export default function ScorecardPage(){
  ...
  return <main style={{minHeight:"100vh",background:COLORS.bg,color:COLORS.text,fontFamily:"Arial, sans-serif"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"minmax(260px,300px) minmax(0,1fr)",gap:10,padding:"4px 18px 18px"}}>
      ...
      <div style={{background:COLORS.panel,minHeight:480,padding:16,borderRadius:"0 0 8px 8px",overflow:"auto"}}>{marketForTab[tab]?<TopTen market={marketForTab[tab]} rows={rankings[marketForTab[tab]]||[]} status={scanStatus[marketForTab[tab]]} progress={scanProgress[marketForTab[tab]]} updated={scanUpdated[marketForTab[tab]]} onScan={scanMarket} onStop={stopScan} onOpen={run}/>:!result?<p>Run an analysis or load the sample company.</p>:tab==="Overview"?<Overview data={data} result={result}/>:tab==="Score Breakdown"?<Breakdown result={result}/>:<pre style={{whiteSpace:"pre-wrap",fontFamily:"Courier New",fontSize:13}}>{JSON.stringify(data,null,2)}</pre>}</div></section>
