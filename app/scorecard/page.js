...
export default function ScorecardPage(){
  const [ticker,setTicker]=useState(""),[data,setData]=useState(null),[macro]=useState(initialMacro),[status,setStatus]=useState("Ready"),[tab,setTab]=useState("Overview"),[loading,setLoading]=useState(false);
  ...
  return <main style={{minHeight:"100vh",background:COLORS.bg,color:COLORS.text,fontFamily:"Arial, sans-serif"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"minmax(260px,300px) minmax(0,1fr)",gap:10,padding:"4px 18px 18px"}}>
      ...
      <section style={{minWidth:0}}><div style={{background:COLORS.panel,padding:18,borderRadius:8,marginBottom:10,textAlign:"center"}}><h2 style={{margin:"0 0 14px"}}>Stock Gauge</h2><div style={{width:"100%",height:28,background:COLORS.panel2,borderRadius:20,overflow:"hidden"}}><div style={{height:"100%",width:`${result?result.total:0}%`,background:COLORS.accent,borderRadius:20}}/></div><div style={{fontSize:34,fontWeight:900,marginTop:12}}>{result?result.total.toFixed(1):"—"}</div></div>
